...
/sbin/service iptables -t nat -A POSTROUTING -p tcp -s {{ lvs.private.ip.network }}/{{ lvs.private.ip.maskbit }} --sport 20 -j MASQUERADE
# Rules for FTP passive connections in LVS (http://red.ht/OBrlF3):
/sbin/service iptables -t mangle -A PREROUTING -p tcp -d {{ lvs.private.ip.addr }}/{{ lvs.private.ip.maskbit }} --dport 21 -j MARK --set-mark 21
/sbin/service iptables -t mangle -A PREROUTING -p tcp -d {{ lvs.private.ip.addr }}/{{ lvs.private.ip.maskbit }} --dport 10000:20000 -j MARK --set-mark 21 
/sbin/service iptables save
